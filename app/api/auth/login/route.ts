...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:partner.soap.sforce.com">
  ...
  try {
    tokenResponse = await fetch(`${loginUrl}/services/Soap/u/61.0`, {
      method: 'POST',
    ...
    return NextResponse.json({ error: 'Could not reach Salesforce. Check the selected login environment.' }, { status: 502 });
  }

  if (!tokenResponse.ok) {
    const responseText = await tokenResponse.text();
    const detail = responseText.match(/<faultstring>(.*?)<\/faultstring>/)?.[1] || 'Salesforce rejected the login. Check your username, password, security token, and login environment.';
